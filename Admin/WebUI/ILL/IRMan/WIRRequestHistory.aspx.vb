...
Namespace eMicLibAdmin.WebUI.ILL
    Partial Class WIRRequestHistory
        Inherits clsWBase
...
#End Region
        ...
        Private Sub CheckFormPermission()
            If Not CheckPemission(153) Then
                Page.RegisterClientScriptBlock("AccessDeniedJS", "<script language = 'javascript'>alert('" & ddlLabelNote.Items(2).Text & "');self.close();</script>")
                Response.End()
            End If
        End Sub
        ...
        Private Sub BindScript()
            If Request("HasMail") & "" <> "" Then
                btnClose.Attributes.Add("OnClick", "opener.top.main.Workform.location.href='WIRMan.aspx';self.close();")
            Else
        ...
        Private Sub BindData()
            ...
            Dim blnHasMail As Boolean = False

            If Not Request.QueryString("ILLID") Is Nothing Then
                If IsNumeric(Request.QueryString("ILLID")) Then
                    objBILLInRequest.ILLID = CLng(Request.QueryString("ILLID"))

                    If Request("HasMail") & "" <> "" Then
                        tblRequestHistory = objBILLInRequest.GetIRHistoryInfor(True)
                    Else
                        tblRequestHistory = objBILLInRequest.GetIRHistoryInfor
                    End If
                    ...
                    If Not tblRequestHistory Is Nothing Then
                        If tblRequestHistory.Rows.Count > 0 Then
                            For intIndex = 0 To tblRequestHistory.Rows.Count - 1
                                ...
                                Select Case intAPDUType
                                    ...
                                    Case 12
                                        If Not IsDBNull(tblRequestHistory.Rows(intIndex).Item("MDUEDATE")) Then
                                            strContentHis = strContentHis & "<DD>" & ddlLabel.Items(30).Text & ": " & tblRequestHistory.Rows(intIndex).Item("MDUEDATE")
                                        End If
                                        If Not IsDBNull(tblRequestHistory.Rows(intIndex).Item("Renewable")) Then
                                    ...
                                    Case 13
                                        If Not IsDBNull(tblRequestHistory.Rows(intIndex).Item("MDUEDATE")) Then
                                            strContentHis = strContentHis & "<DD>" & ddlLabel.Items(26).Text & ": " & tblRequestHistory.Rows(intIndex).Item("MDUEDATE")
                                        End If
                                        strContentHis = strContentHis & "<BR>"
                                    Case 14
                                        If Not IsDBNull(tblRequestHistory.Rows(intIndex).Item("MDUEDATE")) Then
                                            strContentHis = strContentHis & "<DD>" & ddlLabel.Items(28).Text & ": " & tblRequestHistory.Rows(intIndex).Item("MDUEDATE")
                                        End If
                                        If Not IsDBNull(tblRequestHistory.Rows(intIndex).Item("Renewable")) Then
                                            strContentHis = strContentHis & "<br>" & ddlLabel.Items(29).Text & ": "
                                            If Not CInt(tblRequestHistory.Rows(intIndex).Item("Renewable")) = 0 Then
                                                strContentHis = strContentHis & ddlLabel.Items(20).Text
                                            Else
                                                strContentHis = strContentHis & ddlLabel.Items(21).Text
                        ...
                        End If
                    Else
                        Page.RegisterClientScriptBlock("Error2", "<script language='javascript'>alert('" & ddlLabel.Items(0).Text & "')</script>")
                    End If
                Else
                    Page.RegisterClientScriptBlock("Error3", "<script language='javascript'>alert('" & ddlLabel.Items(0).Text & "')</script>")
                End If
            Else
                Page.RegisterClientScriptBlock("Error4", "<script language='javascript'>alert('" & ddlLabel.Items(0).Text & "')</script>")
            End If

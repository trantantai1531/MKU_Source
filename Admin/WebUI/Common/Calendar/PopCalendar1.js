...
function init() {
    if (!ns4) {
        ...
        if (showToday == 1) {
            document.getElementById('lblToday').innerHTML = '<font color="#000066">' + 'Hôm nay là' + ' <a onmousemove="window.status=\'' + gotoString.vie + '\'" onmouseout="window.status=\'\'" title="' + gotoString.vie + '" style="' + styleAnchor + '" href="javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();">' + '<b>' + fulldayName.vie[(today.getDay() - startAt == -1) ? 6 : (today.getDay() - startAt)] + '</b>, ' + dateNow + ' <b>' + monthName.vie[monthNow] + '</b> ' + yearNow + '</a></font>';
        }

        sHTML1 = '<span id="spanLeft" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeLeft\',\'left2.gif\');this.style.borderColor=\'#8af\';window.status=\'' + scrollLeftMessage.vie + '\'" onclick="decMonth()" onmouseout="clearInterval(intervalID1);swapImage(\'changeLeft\',\'left1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartDecMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeLeft" src="' + imgDir + 'left1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
        sHTML1 += '<span id="spanRight" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeRight\',\'right2.gif\');this.style.borderColor=\'#8af\';window.status=\'' + scrollRightMessage.vie + '\'" onmouseout="clearInterval(intervalID1);swapImage(\'changeRight\',\'right1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onclick="incMonth()" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartIncMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeRight" src="' + imgDir + 'right1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
        sHTML1 += '<span id="spanMonth" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeMonth\',\'drop2.gif\');this.style.borderColor=\'#8af\';window.status=\'' + selectMonthMessage.vie + '\'" onmouseout="swapImage(\'changeMonth\',\'drop1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onclick="popUpMonth()"></span>&nbsp;';
        sHTML1 += '<span id="spanYear" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeYear\',\'drop2.gif\');this.style.borderColor=\'#8af\';window.status=\'' + selectYearMessage.vie + '\'" onmouseout="swapImage(\'changeYear\',\'drop1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onclick="popUpYear()"></span>&nbsp;';
...
function constructDate(d, m, y) {
    ...
    sTmp = sTmp.replace('<o>', monthName.vie[m]);
    sTmp = sTmp.replace('<p>', monthName2.vie[m]);
...
function constructCalendar() {
    ...
    if (showWeekNumber == 1) {
        sHTML += '<td width="27"><b>' + weekString.vie + '</b></td><td width="1" rowspan="7" bgcolor="#d0d0d0" style="padding:0px"><img src="' + imgDir + 'divider.gif" width="1"></td>';
    }

    for (i = 0; i < 7; i++) {
        sHTML += '<td width="27" align="right"><b><font color="#000066">' + dayName.vie[i] + '</font></b></td>';
    }
    ...
    for (datePointer = 1; datePointer <= numDaysInMonth; datePointer++) {
        ...
        dateMessage = 'onmousemove="window.status=\'' + selectDateMessage.vie.replace('[date]', constructDate(datePointer, monthSelected, yearSelected)) + '\'" onmouseout="window.status=\'\'" ';
    ...
    document.getElementById('spanMonth').innerHTML = '&nbsp;' + monthName.vie[monthSelected] + '&nbsp;<img id="changeMonth" src="' + imgDir + 'drop1.gif" width="12" height="10" border="0">'
    document.getElementById('spanYear').innerHTML = '&nbsp;' + yearSelected + '&nbsp;<img id="changeYear" src="' + imgDir + 'drop1.gif" width="12" height="10" border="0">';
}

function showCalendar(ctl, ctl2, format, lang, past, fx, fy) {
    if (lang != null && lang != '') vie = lang;
    ...
    if (showToday == 1) {
        document.getElementById('lblToday').innerHTML = '<font color="#000066">' + 'Hôm nay là' + ' <a onmousemove="window.status=\'' + gotoString.vie + '\'" onmouseout="window.status=\'\'" title="' + gotoString.vie + '" style="' + styleAnchor + '" href="javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();">' + dayName.vie[(today.getDay() - startAt == -1) ? 6 : (today.getDay() - startAt)] + ', ' + dateNow + ' ' + monthName.vie[monthNow].substring(0, 3) + ' ' + yearNow + '</a></font>';
    }
...
function popUpCalendar(ctl, ctl2, format, zoneid) {
    ...
    var leftPositionCalendar = getOffset(ctl).left;
   
    var bodyWidth = document.getElementsByTagName("body")[0].offsetWidth;;
 
    if (bodyWidth - leftPositionCalendar < 260) {
        leftpos = -1 *( 260 - (bodyWidth - leftPositionCalendar));
    }

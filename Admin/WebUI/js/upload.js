...
function init_upload(ctrl) {
    var pathLink = document.getElementsByClassName("SelectedTreeNode");
 
    var txtfilePath;
    txtfilePath = document.getElementById('txtfilePath');
    if (pathLink.length == 0) {
        //if (txtfilePath.value == '') {
        alert("Bạn vui lòng chọn đường dẫn lưu tư liệu điện tử.");
        return false;
        //} 
    }

    if (ctrl.GetFiles().length > 0 && pathLink.length > 0) {
        var btn = document.getElementById("btn-upload");
...
function check_uploa_file() {
  
    var resultTable = document.getElementById('upload_file_result');
    if (resultTable == null) {
        alert("Bạn vui lòng upload file.");
        return false;
    }
    else
        return true;
...
function init_upload_Edata(ctrl) {
    var txtfilePath;
    txtfilePath = document.getElementById('txtfilePath');
    if (txtfilePath) {
        if (txtfilePath.value == '') {
...
(new Image()).src = "../../Images/ComponentArt/Upload/images/vertical.png";
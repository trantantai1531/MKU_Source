...
Namespace eMicLibAdmin.WebUI.Cataloguer
    Partial Class WCataSH
        ...
        End Sub
        Private Sub BindJS()
            btnAddSH.Attributes.Add("onclick", "OpenWindow('WAddSH.aspx', 'WAddSH',800,600,200,100)")
        ...
        Private Sub BinDDl()
            'keyword
            Dim tblSH As DataTable = objBSH.GetAllSHBibliography()
            ...
            If tblSH IsNot Nothing AndAlso tblSH.Rows.Count > 0 Then
                For Each row As DataRow In tblSH.Rows
        ...
        Private Sub ConvertTable(ByVal tblResult As DataTable, ByRef tblConvert As DataTable, ByRef totalRecord As Integer, ByRef totalCopyNumber As Integer)
            If (Not IsNothing(tblResult)) AndAlso (tblResult.Rows.Count > 0) Then
                ...
                For Each rows As DataRow In tblResult.Rows
                    ...
                    Dim strItemType As String = rows.Item("ItemType") & ""
        ...
        Private Sub BindData(ByVal intSHId As Integer, Optional ByVal intTypeId As Integer = 0, Optional ByVal intLoanType As Integer = 0)
            Try
                ...
                Dim tblResult As DataTable = objBItem.GetItemBySH(intSHId, intTypeId, intLoanType, yearFrom, yearTo, intAcqSource, intLocation, strAdditionalBy, strDicFaculty, strDateFrom, strDateTo)
        ...
        Private Sub BindData(ByVal strSH As String, Optional ByVal intTypeId As Integer = 0, Optional ByVal intLoanType As Integer = 0)
            Try
                Dim yearFrom As Integer = CType(If(String.IsNullOrEmpty(txtYearFrom.Text), "0", txtYearFrom.Text), Integer)
                Dim yearTo As Integer = CType(If(String.IsNullOrEmpty(txtYearTo.Text), "0", txtYearTo.Text), Integer)
                Dim strAdditionalBy As String = txtAdditionalBy.Text
                ...
                Dim tblResult As DataTable = objBItem.GetItemBySH(strSH, intTypeId, intLoanType, yearFrom, yearTo, intAcqSource, intLocation, strAdditionalBy, strDicFaculty, strDateFrom, strDateTo)
                Dim totalRecord As Integer = 0
                Dim totalCopyNumber As Integer = 0
                ResultSearch(tblResult, totalRecord, totalCopyNumber)
        ...
        Protected Sub btnExportWord_Click(sender As Object, e As EventArgs) Handles btnExportWord.Click
            ...
            If rdDDLSH.Checked = True Then
                tblResult = objBItem.GetItemBySH(CInt(ddlSH.SelectedItem.Value), CInt(ddlItemType.SelectedValue), CInt(ddlLoanType.SelectedValue), yearFrom, yearTo, intAcqSource, intLocation, strAdditionalBy)
            End If
            If rdTextSH.Checked = True Then
                tblResult = objBItem.GetItemBySH(txtSH.Text, CInt(ddlItemType.SelectedValue), CInt(ddlLoanType.SelectedValue), yearFrom, yearTo, intAcqSource, intLocation, strAdditionalBy)
            End If
            ...
            Try
                ...
                For i As Integer = 0 To columnscount - 1
                    ...
                    Select Case tblConvert.Columns(i).ColumnName.ToString()
                        ...
                        Case "ItemType"
                            Response.Write(ddlLabelHeaderTable.Items(10).Text)
                        Case "AcqSource"
                            Response.Write(ddlLabelHeaderTable.Items(11).Text)
                        Case "AdditionalBy"
                            Response.Write(ddlLabelHeaderTable.Items(12).Text)
                        Case "LoanType"
                            Response.Write(ddlLabelHeaderTable.Items(13).Text)
                        Case "LocationName"
                            Response.Write(ddlLabelHeaderTable.Items(14).Text)
                        Case Else

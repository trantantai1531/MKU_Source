<%@ Page Language="VB" AutoEventWireup="false" CodeFile="AcqAttachFileCategory.aspx.vb" Inherits="eMicLibAdmin.WebUI.Catalogue.AcqAttachFileCategory" %>
...
<head runat="server">
    ...
    <!--
        ...
        {
            var strFieldValue = top.main.Sentform.document.forms[0].tag956$a.value;
            var objCheck = document.getElementById(obj);
            if (objCheck.getAttribute("checked") == "checked")
            {
                document.getElementById(obj).setAttribute("checked","");
                strFieldValue = strFieldValue.replace("::" + name + "$&", "");
                strFieldValue = strFieldValue.trim();
            }
            else
            {
                document.getElementById(obj).setAttribute("checked", "checked");
                strFieldValue = strFieldValue + "  ::" + name + "$&";
                strFieldValue = strFieldValue.trim();
            }
            var strName = "";
            var arrFieldValue = strFieldValue.split("$&");
            var tmpFieldValue = "";
            for (var index = 0; index < arrFieldValue.length; index++)
            {
                if (arrFieldValue[index] != "")
                {
                    if (strName == "")
                    {
                        strName = arrFieldValue[index].trim().substring(2);
                    }
                    tmpFieldValue = tmpFieldValue + arrFieldValue[index].trim() + "$&";
                }
            }
            top.main.Workform.document.forms[0].tag956$a.value = '';
            top.main.Workform.document.forms[0].tag956$a.value = strName;
            top.main.Sentform.document.forms[0].tag956$a.value = '';
            top.main.Sentform.document.forms[0].tag956$a.value = tmpFieldValue;

            myUpdateRecord('956$a');

        }
        function createfolder()
        {
            var strFolderAdd = document.getElementById("txtFolderAdd").value;
            create_folder(strFolderAdd);
        ...
        function TreeViewFolder_onNodeSelect(sender, eventArgs) {
            var txtfilePath, hidSFile, hidPathRoot;
            ...
            hidSFile = document.getElementById('hidSFile');
            hidPathRoot = document.getElementById('hidPathRoot');
            var tblData = LoadFolder(hidSFile.value, document.getElementById('txtfilePath').value, hidPathRoot.value);
            document.getElementById("<% =ltrGridViewFile.ClientID %>").innerHTML = tblData;
        }

        function LoadListFile(arrData)
        {
            var strHTML = "";
            strHTML = strHTML + "<table width='100%' border='1'>";
            strHTML = strHTML + "<tr>";
            strHTML = strHTML + "<td width='20px'>&nbsp;</td>";
            strHTML = strHTML + "<td>FileName</td>";
            strHTML = strHTML + "</tr>";
            for (var index = 0; index < arrData.length; index++)
            {
                strHTML = strHTML + "<tr>";
                strHTML = strHTML + "<td><input type='checkbox' id='CheckBoxChecked_" + index + "' onchange='changeFile('CheckBoxChecked_" + index + "','" + arrData[index][0] + "')'></td>";
                strHTML = strHTML + "<td>" + arrData[index][0] + "</td>";
                strHTML = strHTML + "</tr>";
            }
            strHTML = strHTML + "</table>";
            document.getElementById("<% =ltrGridViewFile.ClientID %>").innerHTML = strHTML;
        }
...
<body style="background-color:White;">
    <form id="form1" runat="server">
               ...
               <table id="table1" width="100%" border="0" cellspacing="3" cellpadding="3" bgcolor="white">                                
                        <tr>
                            ...
                            <td  align="left"  style="width:80%">
                                <asp:TextBox ID="txtfilePath" runat="server" Text="" Width="100%" onfocus="this.blur();return false;"></asp:TextBox>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2" align="right">
                                <asp:TextBox ID="txtFolderAdd" runat="server" Width="50%"></asp:TextBox>
                                <input id="btnAddFolder" type="button" value="Thêm thư mục" class="lbButton" onclick="createfolder();" />
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td colspan="2">
                                <asp:Label ID="ltrGridViewFile" runat="server" Text="" Width="100%"></asp:Label>
                                <div style="display:none;">
                                    <asp:Button runat="server" ID="raiseLoadFolder"  Text="raiseLoadFolder"/>  
                                    <asp:Button runat="server" ID="raiseAddFolder"  Text="raiseAddFolder"/>  
                                </div>
                            </td>
                        </tr>
                    </table>
     ...
     <div style="display:none">
        ...
        <input id="hidPathRoot" type="hidden" runat="server"/>

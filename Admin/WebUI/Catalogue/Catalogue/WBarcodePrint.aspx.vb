' class 
' Puspose: Display Barcode Images
' Creator: Sondp
' CreatedDate: 
...
#End Region
        ...
        Private Function Display(ByVal boolPrintMode As Boolean, ByVal boolElse As Boolean, ByVal lngStartID As Long, ByVal lngStopID As Long, ByVal collecBarCodeChoice As Collection) As String
            ...
            If Session("lazerprinter") = False Then ' Barcode printer
                objBT.TemplateID = collecBarCodeChoice.Item("barcodetype")
                objBT.TemplateType = 79
                tblTemplate = objBT.GetTemplate
                If Not tblTemplate Is Nothing Then
                    If tblTemplate.Rows.Count > 0 Then
                        If UBound(objData) > 0 Then
                            For lngi = 0 To UBound(objData)
                                strDisplay &= Replace(tblTemplate.Rows(0).Item("Content"), "<$COPYNUMBER$>", objData(lngi)) & vbCrLf
                            Next
                            ...
                            End If
                            Display = strDisplay.Replace(vbCrLf, "<BR>")
                        End If
                    End If
                End If
            Else ' Lazer printer
                If UBound(objData) >= 0 Then
                    ...
                    objBCC.MakeImgBarcode(objData, collecBarCodeChoice.Item("imagetype"), collecBarCodeChoice.Item("width"), collecBarCodeChoice.Item("height"), collecBarCodeChoice.Item("type"), "", "", "", collecBarCodeChoice.Item("rotation"), strBarcodeColor, bolBarcodeGenerateLabel)
            ...
            If Not objDataToGen Is Nothing Then
                If UBound(objDataToGen) >= 0 Then ' Have data to gen barcode image                  
                    If boolPrintMode = True Then  ' Print from lazer printer
                        strDisplay = "<TABLE cellpadding=0 cellspacing=0><TR>"
                        For lngi = LBound(objDataToGen) To UBound(objDataToGen) 'lngStopID
                            ...
                            Else
                                ...
                                End If
                            End If
                            strDisplay &= "<TD style='border:1px dotted black;border-color:#000000' height='78px' valign='bottom'><IMG src=../../Common/WPrintBarCode.aspx?i=" & lngi + lngStartPosition & "&ImgType=" & intImageType & " &rotate=" & intRotation & "></TD>"

...
#End Region
        ...
        Private Sub BindScript()
            ...
            Me.SetCheckNumber(txtSize, ddlLabel.Items(15).Text)
        End Sub
        ...
        Private Sub ShowTreeView(ByVal intType As Integer)
            ...
            Select Case intType
                Case 1
                    ' View by Directory
                    tblTemp = objBEData.GetDisplayTypes(1, intFree)
                    lblScript.Visible = True
                Case 2
                    ' View by Status
                    tblTemp = objBEData.GetDisplayTypes(2, intFree)
                    lblScript.Visible = True
                Case 3
                    ' View by File Type
                    tblTemp = objBEData.GetDisplayTypes(3, intFree)
                    lblScript.Visible = True
                Case 4
                    ' View by Collection
                    tblTemp = objBEData.GetDisplayTypes(4, intFree)
                    lblScript.Visible = True
                Case 5
                    ' Search
            ...
            If intType = 1 Then
                ...
                If Not objSysPara(7) Is Nothing Then
                    ...
                    End If
                End If
                'Dim arrLocation() As String = Me.GetOneParaSystem("EDATA_LOCATIONS").Replace(",", ";").Split(";")
                'For intIndex = 0 To arrLocation.Length - 1
                '    If arrLocation(intIndex) <> "" And InStr(arrLocation(intIndex), ":\") > 0 Then
                '        arrRootNodes = Split(arrLocation(intIndex), "\")
                '        If InStr(LCase(strRootFolders), "," & LCase(arrRootNodes(0)) & ",") = 0 Then
                '            strRootFolders = strRootFolders & arrRootNodes(0) & ","
                '            ReDim Preserve arrRootFolders(intMax)
                '            arrRootFolders(intMax) = arrRootNodes(0)
                '            intMax = intMax + 1
            ...
            If intType = 1 Then
                ...
                If Not arrRootFolders Is Nothing AndAlso arrRootFolders.Length > 0 Then
                    For intIndex = 0 To arrRootFolders.Length - 1
                ...
                If Not arrTemp Is Nothing AndAlso arrTemp.Length > 0 Then
                    For intIndex = 0 To arrTemp.Length - 1
        ...
        Protected Function BuildDirectoryTree(ByVal intType As Integer, ByVal tblSource As DataTable, Optional ByVal strPath As String = "", Optional ByVal intDepth As Integer = 0) As String
            ...
            If intType = 1 Then
                ...
                If intDepth = 0 Then
                    strTree = strTree & "aux" & intDepth + 1 & " = insFld(foldersTree, gFld('" & Replace(strPath, "\", "\\") & "', 'WShowDetail.aspx?Loc=" & Replace(strPath, "\", "\\") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'))" & Chr(10)
                ...
                Else
                    strTree = strTree & "aux" & intDepth + 1 & " = insFld(aux" & intDepth & ", gFld('" & objDirInfor.Name & "', 'WShowDetail.aspx?Loc=" & Replace(strPath, "\", "\\") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'))" & Chr(10)
                    strTree = strTree & "aux" & intDepth + 1 & ".iconSrc = ICONPATH + 'ftv2folderopen.gif'" & Chr(10)
                    strTree = strTree & "aux" & intDepth + 1 & ".iconSrcClosed = ICONPATH + 'ftv2folderclosed.gif'" & Chr(10)
                    strTree = strTree & "aux" & intDepth + 1 & ".targetFrame = ""foldercontents""" & Chr(10)
                End If
                ...
                For Each subDir In objDirInfor.GetDirectories
                    Dim i As Integer
                    dtrow = tblSource.Select("FileLocation LIKE '" & strPath & "\" & subDir.Name & "\%' OR FileLocation = '" & strPath & "' OR FileLocation = '" & strPath & "\" & subDir.Name & "'")
                            ...
                            Select Case intType
                                Case 2
                                    Select Case tblSource.Rows(intIndex).Item("Status")
                                        Case 1
                                            strParam = "'" & ddlLabel.Items(8).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=1&Type=1&Refresh=1&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 2
                                            strParam = "'" & ddlLabel.Items(9).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=1&Type=2&Refresh=1&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 3
                                            strParam = "'" & ddlLabel.Items(10).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=1&Type=3&Refresh=1&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 4
                                            strParam = "'" & ddlLabel.Items(11).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=1&Type=4&Refresh=1&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                    End Select
                                Case 3
                                    Select Case tblSource.Rows(intIndex).Item("MediaType")
                                        Case 1
                                            strParam = "'" & ddlLabel.Items(1).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=2&Type=1&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 2
                                            strParam = "'" & ddlLabel.Items(2).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=2&Type=2&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 3
                                            strParam = "'" & ddlLabel.Items(3).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=2&Type=3&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 4
                                            strParam = "'" & ddlLabel.Items(4).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=2&Type=4&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 5
                                            strParam = "'" & ddlLabel.Items(5).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=2&Type=5&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 6
                                            strParam = "'" & ddlLabel.Items(6).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=2&Type=6&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Case 7
                                            strParam = "'" & ddlLabel.Items(7).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=2&Type=7&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                    End Select
                                Case 4
                                    ' Declare variables
                                    ...
                                    If IsDBNull(tblSource.Rows(intIndex).Item("CollectionID")) Then
                                        strParam = "'" & ddlLabel.Items(13).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=3&CollectionID=-1&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                    Else
                                        ...
                                        If Not strCollectionName = "" Then
                                            strParam = "'" & strCollectionName & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=3&CollectionID=" & tblSource.Rows(intIndex).Item("CollectionID") & "&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        Else
                                            strParam = "'" & ddlLabel.Items(12).Text & " (" & tblSource.Rows(intIndex).Item("NOR") & ")','WShowDetail.aspx?ViewType=3&CollectionID=" & tblSource.Rows(intIndex).Item("CollectionID") & "&BrowseOnly=" & Request("BrowseOnly") & "&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "'"
                                        End If
        ...
        Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
            ...
            If Not strPhysicalPath = "" Then
                strWhere = strWhere & " AND PhysicalPath like '" & strPhysicalPath & "'"
            End If
            ...
            strWhere = strWhere & " AND Free =" & intFree
            ...
            Page.RegisterClientScriptBlock("LoadSearchResult", "<script language='javascript'>parent.foldercontents.location.href='WShowDetail.aspx?Search=1&FieldCode=" & Request("FieldCode") & "&Repeatable=" & Request("Repeatable") & "&WField=" & Request("WField") & "&SField=" & Request("SField") & "';</script>")
        End Sub

...
' Puspose: Management renews patron card
' Creator: Oanhtn
...
#Region " Web Form Designer Generated Code "
        ...
        End Sub
        Protected WithEvents lblValidDate As System.Web.UI.WebControls.Label
        Protected WithEvents lblRenewDate As System.Web.UI.WebControls.Label
        Protected WithEvents lblPickPatron As System.Web.UI.WebControls.Label
        Protected WithEvents lblWomen As System.Web.UI.WebControls.Label
        Protected WithEvents lblMen As System.Web.UI.WebControls.Label
        Protected WithEvents lblAllSex As System.Web.UI.WebControls.Label
        Protected WithEvents lblSexValue As System.Web.UI.WebControls.Label
        Protected WithEvents lblChoice As System.Web.UI.WebControls.Label
...
#End Region
        ...
        ' Method: LoadBackData
        ' Popurse: This method used to Init 3 javascript Array and load back data into suite dropdonwlist
        Private Sub LoadBackData()
            ...
            Try
                ...
                If Not tblTemp Is Nothing AndAlso tblTemp.Rows.Count > 0 Then
                    ...
                    strTempo = strTempo & "var ArrOptionIndex = new Array(" & intMax & ");" & Chr(10)
                    strTempo = strTempo & "var ArrValue = new Array(" & intMax & ");" & Chr(10)
                    strTempo = strTempo & "var ArrText = new Array(" & intMax & ");" & Chr(10)
        ...
        Private Sub BindJS()
            ...
            Page.RegisterClientScriptBlock("WRenewCardsJs", "<script language = 'javascript' src='js/WRenewCards.js'></script>")
            ...
            Me.RegisterCalendar("..")
            SetOnclickCalendar(lnkFromDate1, txtFieldValueFrom1, ddlLabel.Items(10).Text)
            SetOnclickCalendar(lnkFromDate2, txtFieldValueFrom2, ddlLabel.Items(10).Text)
            SetOnclickCalendar(lnkToDate1, txtFieldValueTo1, ddlLabel.Items(10).Text)
            SetOnclickCalendar(lnkToDate2, txtFieldValueTo2, ddlLabel.Items(10).Text)
            SetOnclickCalendar(lnkExtendedDate, txtExtendedDate, ddlLabel.Items(10).Text)

            ' Check Valid Date
            txtExtendedDate.ToolTip = Session("DateFormat")
            ...
            Try
                Select Case bytOption
                    ...
                    Case 7 ' PatronGroup
                        ...
                        strTextField = "Name"
                    Case Else 'Sex
                        Dim ArrSexTextField() As String = {ddlLabel.Items(9).Text, ddlLabel.Items(7).Text, ddlLabel.Items(8).Text}
        ...
        Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
            ...
            End If
            If Not blnFalg Then
                Page.RegisterClientScriptBlock("PatronNotFound", "<script language = 'javascript'>alert('" & ddlLabel.Items(11).Text & "')</script>")
            ...
            Else
                ...
                Me.ShowWaitingOnPage(ddlLabel.Items(12).Text & " " & CStr(UBound(strArrResult) + 1) & " " & ddlLabel.Items(13).Text, "..")
        ...
        ' Event: btnExtend_Click
        Private Sub btnExtend_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExtend.Click
            ...
            For Each dtgItem In dtgResult.Items ' Get Selected Patron
                ...
                If chkSelected.Checked Then
                    'Check RenewDate
                    If Trim(txtExtendedDate.Text) <> "" Then
                        objBPC.NewExpiredDate = Trim(txtExtendedDate.Text)
                        tblPatronInfor = objBPC.CheckRenewDate(CInt(CType(dtgItem.FindControl("lblID"), Label).Text))
                        If CInt(tblPatronInfor.Rows(0).Item("intByte")) = 0 Then
            ...
            Try
                If intCheck > 0 Then
                    Page.RegisterClientScriptBlock("ErrJS", "<script language='javascript'>alert('" & ddlLabel.Items(14).Text & "');</script>")
                End If
                If Not strSelectedIDs = "" Then
                    strSelectedIDs = Left(strSelectedIDs, strSelectedIDs.Length - 1)
                    objBPC.NewExpiredDate = Trim(txtExtendedDate.Text)
                    objBPC.Years = ddlExtendedYear.SelectedItem.Value
                    objBPC.Months = ddlExtendedMonth.SelectedItem.Value
                    objBPC.PatronIDs = strSelectedIDs
                    objBPC.ReNew() ' Renew Card(s)
                    ' WriteLog
                    Call WriteLog(29, ddlLabel.Items(3).Text, Request.ServerVariables("SCRIPT_NAME"), Request.ServerVariables("REMOTE_ADDR"), clsSession.GlbUserFullName)
                    If intCheck > 0 Then
                        Page.RegisterClientScriptBlock("ErrJS", "<script language='javascript'>alert('" & ddlLabel.Items(14).Text & "');</script>")
                    Else
                        Page.RegisterClientScriptBlock("ErrJS", "<script language='javascript'>alert('Gia hạn thành công');</script>")

                    End If


                    Call GridDisplay(CType(Session("strPatronIDs"), Object), Session("strSQLIDs")) ' Refresh DataGrid
                    dtgResult.Rebind()
                End If
                ' Load Default for ddlOptionFieldValue1, ddlOptionFieldValue1
                Call LoadData(1, 5)
        ...
        Protected Sub btnExtendAll_Click(sender As Object, e As EventArgs) Handles btnExtendAll.Click
            Dim dtgItem As GridDataItem
            Dim chkSelected As HtmlInputCheckBox
            Dim strSelectedIDs As String = ""
            Dim tblPatronInfor As DataTable
            ...
            For Each dtgItem In dtgResult.Items ' Get Selected Patronar
                'Check RenewDate
                If Trim(txtExtendedDate.Text) <> "" Then
                    objBPC.NewExpiredDate = Trim(txtExtendedDate.Text)
                    tblPatronInfor = objBPC.CheckRenewDate(CInt(CType(dtgItem.FindControl("lblID"), Label).Text))

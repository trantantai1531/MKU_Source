...
#End Region
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            If Not Page.IsPostBack Then
                ...
                If Request.QueryString("xLabel") & "" = "" Then
                    Call GenChart()
                Else
                    Session("month") = CStr(Request.QueryString("xLabel"))
                    Page.RegisterClientScriptBlock("StatisticAcqDateJs", "<script language='javascript'> parent.parent.mainacq.location.href='WStatDayFrame.aspx?month=" & CStr(Request.QueryString("xLabel")) & "&year=" & CStr(Session("year")) & "'</script>")
                End If
            End If
        End Sub
        ...
        Private Sub BindScript()
            ...
            Page.RegisterClientScriptBlock("WStatMonthJs", "<script language='javascript' src='../Js/Statistic/WStatMonth.js'></script>")
        End Sub
        ...
        Private Sub GenChart()
            ...
            Try
                If Request.QueryString("year") & "" = "" Then
                    ...
                    Else
                        Session("year") = CStr(Session("year"))
                    End If
                Else ' Have request to
                    Session("year") = CStr(Request.QueryString("year"))
                End If
                lblDAPHTitle.Text = lblDAPHTitle.Text & Session("year")
                lblDAPTitle.Text = lblDAPTitle.Text & Session("year")
                lblBAPHTitle.Text = lblBAPHTitle.Text & Session("year")
                lblBAPTitle.Text = lblBAPTitle.Text & Session("year")
                lblHMoney.Text = lblHMoney.Text & Session("year")
                lblTMoney.Text = lblTMoney.Text & Session("year")
                objBS.LibID = clsSession.GlbSite
                objBS.StatMonth(lblDAPTotal.Text, lblBAPTotal.Text, lblInMonth.Text, Session("year"))
                ...
                If objLabel(0) = -1 Then ' Don't have data
                    Response.Redirect("WStatEmty.aspx")
                Else
                    ' Statistic for DAP
                    ...
                    objBCC.Makebarchart(objData, objLabel, strVtitle, strHtitle, 0, strImg, "WStatMonth.aspx")
                    Session("chart1") = Nothing
                    Session("chart1") = objBCC.OutPutStream
                    objBCC.Makepiechart(objData, objData, strTitle, strImg)
                    ...
                    objBCC.Makebarchart(objDataNext, objLabel, strVtitle, strHtitle, 0, strImg, "WStatMonth.aspx")
                    Session("chart3") = Nothing
                    Session("chart3") = objBCC.OutPutStream
                    Response.Write("<MAP NAME=""map2"">" & objBCC.OutMapImg & "</MAP>")
                    objBCC.Makepiechart(objDataNext, objData, strTitle, strImg)
                    ...
                    objBCC.Makebarchart(objMoney, objLabel, strVtitle, strHtitle, 0, strImg, "WStatMonth.aspx")
        ...
        Private Sub BindData()
            ...
            objBS.LibID = clsSession.GlbSite
            Dim tblData As DataTable = objBS.StatMonthDetail(Session("year"))

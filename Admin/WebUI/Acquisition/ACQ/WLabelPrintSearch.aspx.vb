...
#End Region
        ...
        Private objBHolding As New eMicLibAdmin.BusinessRules.Catalogue.clsBHolding
        ...
        Private Sub BindJS()
            ...
            Page.RegisterClientScriptBlock("WLabelPrintSearchJs", "<script language='javascript' src='../Js/ACQ/WLabelPrintSearch.js'></script>")
            ...
            hrfFromCodeItem.NavigateUrl = "javascript:openSearchItem=window.open('WSearchItem.aspx?ControlName=txtFromCodeItem','SearchCode', 'width=600,height=400,left=100,top=20,resizable=yes,scrollbars=yes');openSearchItem.focus();"
            hrfToCodeItem.NavigateUrl = "javascript:openSearchItem=window.open('WSearchItem.aspx?ControlName=txtToCodeItem','SearchCode', 'width=600,height=400,left=100,top=20,resizable=yes,scrollbars=yes');openSearchItem.focus();"
        End Sub
        ...
        Private Sub BindData()
            ...
            If Not tblTem Is Nothing Then
                If tblTem.Rows.Count > 0 Then
                    ddlLibrary.DataSource = InsertOneRow(tblTem, "           ")
            ...
            objBHolding.LibID = clsSession.GlbSite
            objBHolding.UserID = If(Session("UserID"), 0)
            tblTem = objBHolding.GetHoldingByStatus("TT001")
            Dim newLine As Integer = 0
            If Not tblTem Is Nothing AndAlso tblTem.Rows.Count > 0 Then
                Dim sb As New StringBuilder(1024)
                For Each i As DataRow In tblTem.Rows
                    Dim str As String = i.Item("CopyNumber").ToString().Trim()
                    If str <> "" Then
                        If newLine > 5 Then
                            sb.Append(vbCrLf)
                            newLine = 0
                ...
                If sb.Length > 0 Then
                    ...
                    btnClearQueue.Visible = True
                Else
                    btnClearQueue.Visible = False
                End If
                txtElse.Text = sb.ToString()
            Else
                btnClearQueue.Visible = False
            End If
            tblTem = Nothing
        ...
        Private Sub btnClearQueue_Click(sender As Object, e As EventArgs) Handles btnClearQueue.Click
            Dim result As Integer = objBHolding.ClearHoldingPrintLabel(txtElse.Text.Replace(vbCrLf, ""))
            If result = 1 Then
                txtElse.Text = ""
                btnClearQueue.Visible = False
                ClientScript.RegisterClientScriptBlock(Me.GetType(), "ClearSucc", "alert('Xóa dữ liệu hàng đợi in nhãn thành công');", True)
            ElseIf result = -1 Then
                txtElse.Text = ""
                btnClearQueue.Visible = False
                ClientScript.RegisterClientScriptBlock(Me.GetType(), "ClearEmpty", "alert('Dữ liệu hàng đợi trống!');", True)
            Else
                ClientScript.RegisterClientScriptBlock(Me.GetType(), "ClearFail", "alert('Xóa dữ liệu hàng đợi in nhãn thất bại!!!');", True)
            End If
        End Sub
    End Class
End Namespace
...
Namespace eMicLibLogin
    Public Class clseMicLibLogin
        ...
        Private Function SetPathLogin() As String
            ...
            'Dim str2 As String
            'Dim str3 As String
            'Dim info As New FileInfo("C:\Program Files\DGSoft\WLoginDB.xml")
            'If Not info.Exists Then
            '    Return (Environment.SystemDirectory & "\WLoginDB.xml")
            'End If
            'Return "C:\Program Files\DGSoft\WLoginDB.xml"
            'Dim strArray As String() = str2.Replace(ChrW(10), "").Split(New Char() {ChrW(13)})
            'Dim num2 As Integer = (strArray.Length - 2)
            'Dim i As Integer = 0
            'Do While (i <= num2)
            '    If ((strArray(i).Length > 8) AndAlso (StringType.StrCmp(strArray(i).Substring(0, 8), "WEBLIBOL", False) = 0)) Then
            '        Dim str4 As String = strArray(i).Substring(9)
            '        str3 = str4.Substring(0, (Strings.InStr(str4, ";", 0) - 1))
            '        Exit Do
            '    End If
            '    i += 1
            'Loop
            'info = New FileInfo((str3 & "\WebApplication\WLoginDB.xml"))
            'If info.Exists Then
            '    Return (str3 & "\WebApplication\WLoginDB.xml")
            'End If
            'Return (Environment.SystemDirectory & "\WLoginDB.xml")
            'info = Nothing
            'Return str
        ...
        ' Fields
        Public Const fileInstallLog As String = "\eMicLib60Install.log"

...
Namespace eMicLibOPAC.WebUI.OPAC
    Public Class OBrowseMore
        ...
        Private Sub BindDictionary(ByVal DicID As Integer)
            ...
            Dim i As Integer
            strDic = "0-9  A  Ă  Â  B  C  D  Ð  E  Ê  F  G  H  I  J  K  L  M  N  O  Ô  Ơ  P  Q  R  S  T  U  Ư  V  W  X  Y  Z ~"
            arrDic = strDic.Split("  ")
            For i = 0 To arrDic.Length - 1
                strLable &= "<li>"
                strLable &= "<a href=""OBrowseMore.aspx?DicID=" & Session("DicID") & "&Word=" & arrDic(i) & """>" & arrDic(i) & "</a>"
                strLable &= "</li>"
            Next
            strLable &= "</ul>"
            lrtDictionary.Text = strLable
            If DicID = 9 Or DicID = 12 Or DicID = 14 Then 'nam xuat ban, danh muc va bo suu tap
                lrtDictionary.Visible = False
            Else
                lrtDictionary.Visible = True
            End If
            If DicID = 12 Or DicID = 14 Then
        ...
        '        If Not IsNothing(clsSession.GlbSQLStatementFilter) AndAlso clsSession.GlbSQLStatementFilter <> "" Then
        '            strSQLStatement = clsSession.GlbSQLStatementFilter
        '        Else
        '            strSQLStatement = clsSession.GlbSQLStatement
        '        End If
        ...
        '                objBFilterBrowse.Ids = objBSearchQr.getFormingSQLForID(strSQLStatement)
        '                tblTemp = objBFilterBrowse.GetAllBrowseMoreByWord(strFielSort, strMethodSort, 0)
        '                strResult &= getDictionaryString(DicID, spDocType.InnerText, tblTemp)
        ...
        '                objBFilterBrowse.Ids = objBSearchQr.getFormingSQLForID(strSQLStatement)
        '                tblTemp = objBFilterBrowse.GetAllBrowseMoreByWord(strFielSort, strMethodSort, 0)
        '                strResult &= getDictionaryString(DicID, spTitle.InnerText, tblTemp)
        '                'Case 12
        '                '    Dim dt As New DataTable
        '                '    objBFilterBrowse.Words = strSearch
        '                '    dt = objBFilterBrowse.GetTBrowseTreeviewDDC()
        '                '    If Not IsNothing(dt) AndAlso dt.Rows.Count > 0 Then
        '                '        strResult &= getTriewviewDDC(dt) 'getDDC(dt)
        '                '    Else
        ...
        '                End If
        '        End Select
        '        ltrBrowseMoreList.Text = strResult
            ...
            Try
                ...
                Select Case DicID
                    ...
                    Case 6 'Language
                        ...
                        objBFilterBrowse.Ids = objBSearchQr.getFormingSQLForID(strSQLStatement)
                        tblTemp = objBFilterBrowse.GetAllBrowseMoreByWord(strFielSort, strMethodSort, 0)
                        strResult &= getDictionaryString(DicID, spLanguage.InnerText, tblTemp, strIconCss)
                    Case 7 'NLM
                        strIconCss = " class=""mif-flow-cascade"" "
                        strFielSort = "NumberNo"
                        strMethodSort = "DESC"
                        Call getSort(strFielSort, strMethodSort)
                        objBFilterBrowse.DicID = DicID
                        ...
                        objBFilterBrowse.Ids = objBSearchQr.getFormingSQLForID(strSQLStatement)
                        tblTemp = objBFilterBrowse.GetAllBrowseMoreByWord(strFielSort, strMethodSort, 0)
                        strResult &= getDictionaryString(DicID, spNLM.InnerText, tblTemp, strIconCss)
                    Case 8 'spDDC
                        strIconCss = " class=""mif-flow-cascade"" "
                        strFielSort = "NumberNo"
                        strMethodSort = "DESC"
                        ...
                        objBFilterBrowse.Ids = objBSearchQr.getFormingSQLForID(strSQLStatement)
                        tblTemp = objBFilterBrowse.GetAllBrowseMoreByWord(strFielSort, strMethodSort, 0)
                        strResult &= getDictionaryString(DicID, spDDC.InnerText, tblTemp, strIconCss)
                    Case 9 'publisher year
                    ...
                    Case 14
                        ...
                        If Not tblTemp Is Nothing AndAlso tblTemp.Count > 0 Then
                            ...
                            strResult &= "<div id='treeviewDDC' class='treeview' data-role='treeview'>"
                            strResult &= "<ul>"
                            getCollections(tblTemp, 0, strResult)
                            strResult &= "</ul>" 'Close treeview
                            strResult &= "</div>" 'Close div
        ...
        Private Sub getCollections(ByVal dv As DataView, ByVal intLevel As Integer, ByRef strResultOut As String)
            Try
                ...
                If dv.Count > 0 Then
                    ...
                    For Each rowView In dv
                        ...
                        If boldviewLeaf Then
                            strResultOut &= "<li><a href='#' onclick=""gotoShowRecord(14,'" & spCollection.InnerText & "'," & rowView.Item("ID") & ",1)"">" & "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src='" + strImageCover + "' class='icon' style='width:10%;heigth:10%;'>&nbsp;" & clsUICommon.HightLightText(rowView.Item("DisplayEntry"), strSearch) & " (" & FormatNumber(rowView.Item("NumberNo"), 0) & " <span class='icon-book'></span>)"
                        Else
                            strResultOut &= "<li class='node expanded'><a href='#' onclick=""gotoShowRecord(14,'" & spCollection.InnerText & "'," & rowView.Item("ID") & ",1)""><span class='node-toggle'></span>" & "<img src='" + strImageCover + "' class='icon' style='width:10%;heigth:10%;'>&nbsp;" & clsUICommon.HightLightText(rowView.Item("DisplayEntry"), strSearch) & " (" & FormatNumber(rowView.Item("NumberNo"), 0) & " <span class='icon-book'></span>)"
                        End If
                        strResultOut &= "</a>"
                        Call getCollections(dview, rowView.Item("ID"), strResultOut)
                        strResultOut &= "</li>"
                    Next
                    strResultOut &= "</ul>"
                End If
                ...
                For i As Integer = 0 To iCount
                    ...
                    If IsDBNull(dt.Rows(i).Item("ParentID")) Then
                        ...
                        If bolLeafNode Then
                            ...
                            strResult &= "<li><a href='#' onclick=""gotoShowRecord(12,'" & dt.Rows(i).Item("codeFull") & "')"">" & dt.Rows(i).Item("Caption") & " (" & dt.Rows(i).Item("NumberNo") & " <span class='icon-book'></span>)" 'leaf tag
                            strResult &= "</a></li>"
                        Else
                            strResult &= "<li class='node expanded'>"
                            strResult &= "<a href='#' onclick=""gotoShowRecord(12,'" & dt.Rows(i).Item("Code") & "')""><span class='node-toggle'></span>" & dt.Rows(i).Item("Caption") & " (" & dt.Rows(i).Item("NumberNo") & " <span class='icon-book'></span>)"
                            strResult &= "</a>"
                            strResult &= "<ul>"
                        End If
                    Else
        ...
        '                    strResult &= "<ul>"
        '                    strResult &= "<li class='node expanded'>"
        '                    strResult &= "<a href='#' onclick=""gotoShowRecord(12,'" & rowView.Item("Code") & "')""><span class='node-toggle'></span>" & clsUICommon.HightLightText(rowView.Item("Caption"), strSearch) & " (" & rowView.Item("NumberNo") & " <span class='icon-book'></span>)"
        '                    strResult &= "</a>"
            ...
            Try
                ...
                If dv.Count > 0 Then
                    ...
                    For Each rowView In dv
                        ...
                        dviewLeaf.RowFilter = ""
                        If boldviewLeaf Then
                            strResultOut &= "<li><a href='#' onclick=""gotoShowRecord(12,'" & rowView.Item("codeFull") & "')"">" & clsUICommon.HightLightText(rowView.Item("Caption"), strSearch) & " (" & rowView.Item("NumberNo") & " <span class='icon-book'></span>)"
                        Else
                            strResultOut &= "<li class='node expanded'><a href='#' onclick=""gotoShowRecord(12,'" & rowView.Item("Code") & "')""><span class='node-toggle'></span>" & clsUICommon.HightLightText(rowView.Item("Caption"), strSearch) & " (" & rowView.Item("NumberNo") & " <span class='icon-book'></span>)"
                        End If
                        strResultOut &= "</a>"
        ...
        '                strResult &= "<a class='list' onclick=""gotoShowRecord(" & dicId & ",'" & dicName & "'," & dv.Item(intCount).Row("ID") & ",1)"">"
        ...
        '                strResult &= clsUICommon.HightLightText(dv.Item(intCount).Row("DisplayEntry"), strSearch) & " (" & FormatNumber(dv.Item(intCount).Row("NumberNo"), 0) & " <span class='icon-book'></span>)"
        '                strResult &= "</span>"
        '                strResult &= "</div>"
        '                strResult &= "</a>"
        '            Next
            ...
            Try
                If Not IsNothing(dv) AndAlso dv.Count > 0 Then
                    ...
                    For intCount = intStart To intStop
                        ...
                        'strResult &= "</a>"
                        strResult &= "<li>"
                        strResult &= "<a  onclick=""gotoShowRecord(" & dicId & ",'" & dicName & "'," & dv.Item(intCount).Row("ID") & ",1)"" style=""cursor:pointer;"">"
        ...
        '    Catch ex As Exception
        '    End Try
        '    ltrBrowseMoreOrderBy.Text = strResult
        'End Sub
            ...
            Try
                ...
                Select Case intDicID
                    ...
                    Case 12 'DDC
                        'strDicName = spAuthor.InnerText
                End Select
                ltrBrowseMoreList.Text = getDictionaryString(intDicID, strDicName, dv)
            Catch ex As Exception
            End Try
        End Sub
        ...
        Private Sub BindScript()
            'Page.RegisterClientScriptBlock("SelfJs", "<script language = 'javascript' src = 'js/OBrowseMore.js'></script>")
        End Sub
